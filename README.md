...
```bash
sudo bash -c "$(curl -fsSL 'https://raw.githubusercontent.com/yjrszcq/xdsrun-auto-login/refs/heads/main/install.sh')"
```
...
默认内容类似。配置项名称始终相同，但注释语言会跟随安装时的语言选择切换。
...
如果安装时选择 English，则 watchdog 在配置缺失、二进制不存在等异常场景下输出的错误信息也会是英文。
...
如果安装时选择 English，对应提示会变为：

```text
Re-download and install xdsrun? [y/N]:
```
...
```text
Re-generate xdsrun-watchdog script? [y/N]:
```
